...
struct {
    ...
    __type(value, __u32); // trace_id type. 
    //TODO change value to 64 bit
...
#define bpf_trace_id_set(trace_id)                        \
    ({                                                    \
        __u32 __z = 0;                                    \
        __u32 *__cache = map_lookup_elem(&trace_id_map, &__z); \
...
static __always_inline __u32 load_trace_id() {
...
static __always_inline void check_and_store_trace_id(struct __ctx_buff *ctx) {
    __u32 trace_id = trace_id_from_ctx(ctx);

    // Check if the trace ID is valid
    if (trace_id > 0) {
        bpf_trace_id_set(trace_id);
    }
    bpf_trace_id_set(trace_id); //for now still save the trace_id so we can see the parsing error
    //TODO: zero'ing out
}
