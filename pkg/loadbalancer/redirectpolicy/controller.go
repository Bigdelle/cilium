...
func (c *lrpController) updateRedirects(wtxn writer.WriteTxn, ws *statedb.WatchSet, cleanup func(writer.WriteTxn), lrp *LocalRedirectPolicy, pods []podInfo) func(writer.WriteTxn) {
	...
	case lrpConfigTypeAddr:
		...
		for _, feM := range lrp.FrontendMappings {
			if len(pods) == 0 && !lrp.ForceRedirect {
...
func (c *lrpController) updateRedirectBackends(wtxn writer.WriteTxn, lrp *LocalRedirectPolicy, pods []podInfo) {
	...
	// If no backends were found and ForceRedirect is enabled, add a dummy backend.
	if len(beps) == 0 && lrp.ForceRedirect {
		dummyIP, err := cmtypes.ParseAddrCluster("169.254.169.254")
		if err == nil {
			for _, bePort := range lrp.BackendPorts {
				addr := lb.NewL3n4Addr(bePort.l4Addr.Protocol, dummyIP, bePort.l4Addr.Port, lb.ScopeExternal)
				beps = append(beps, lb.BackendParams{
					Address: addr,
					State:   lb.BackendStateActive,
					PortNames: func() []string {
			...
			c.p.Log.Error("Failed to parse dummy IP for ForceRedirect", logfields.Error, err)
		}
	}
